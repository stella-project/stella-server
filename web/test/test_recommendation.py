...
def generate_feedback(number_of_feedbacks, session_start_date, session_end_date):
    for f in range(0, number_of_feedbacks):
        ...
        click_dict = {
            ...
            "10": {"datasetid": "doc15", "clicked": False, "date": None, "type": "BASE"}
        }
...
def get_site_info_token(client, token, site):
    ...
    rv = client.get(''.join(['/stella/api/v1/sites/', site]), headers={"Authorization": f"Bearer {token}"})
    return json.loads(rv.data)
...
def test_post_results(client):
    ...
    for feedback in feedbacks:
        ...
        rv = client.post('/stella/api/v1/feedbacks/' + str(feedback_id) + '/recommendations',
                         headers={"Authorization": f"Basic {credentials}"},
                         data=result)
        assert 200 == rv.status_code
