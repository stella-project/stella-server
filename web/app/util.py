...
def makeComposeFile():
    ...
    """
    systems = db.session.quary(System).filter_by().all()
    compose = {
        "version": "3",
...
def create_stella_app_yaml(type="all", token=None):
    ...
    if type == "all":
        ...
        systems = db.session.quary(System).filter_by(status="running").all()
        repo_name = "stella-app"
    if type == "rec":
        yml_path = "uploads/gesis.yml"
        systems = db.session.quary(System).filter_by(type="REC", status="running").all()
        repo_name = "stella-app"
    if type == "rank":
        yml_path = "uploads/livivo.yml"
        systems = (
            db.session.quary(System).filter_by(type="RANK", status="running").all()
        )
        repo_name = "stella-app"
