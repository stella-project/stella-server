...
class Session(db.Model):
    ...
    def from_json(json_feedback):
        try:
            start = json_feedback.get("start")
            end = json_feedback.get("end")
            if start is None or end is None:
                raise ValueError("Missing 'start' or 'end' timestamp in feedback.")

            interleave = json_feedback.get("interleave", False)
            if isinstance(interleave, str):
                interleave = interleave.lower() == "true"
            return Feedback(
            start=start,
            end=end,
            interleave=interleave,
            session_id=json_feedback.get("session_id"),
            site_id=json_feedback.get("site_id")
        )
        except Exception as e:
            print("Error in from_json:", e)
            raise e
...
class Feedback(db.Model):
    ...
    def from_json(json_feedback):
        ...
        if isinstance(clicks_raw, dict):
            clicks = clicks_raw
        elif isinstance(clicks_raw, str):
            clicks = json.loads(clicks_raw)
        else:
            raise ValueError("Invalid type for clicks")
